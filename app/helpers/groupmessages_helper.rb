module GroupmessagesHelper
  def non_read_groupmessage
    ...
    groupmember.each do |gm|
      non_read_messagecount = non_read_messagecount + GroupmessageReadCheck.where(group_id: gm.group_id, user_id: current_user.id, read: false).count
  ...
  def non_read_groupmessage_by_group(group_id)
    ...
    GroupmessageReadCheck.where(group_id: group_id, user_id: current_user.id, read: false).count
  end
